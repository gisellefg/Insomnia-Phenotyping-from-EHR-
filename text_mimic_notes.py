...
def main(note_ids_path, mimic_path, output_path):
    ...
    #For the sake of runnning an example we first discard the gzip (.gz)
    #TODO: remember to put on the files when i have access to mimic

    df_mimic_note = pd.read_csv(
        os.path.join(mimic_path, 'NOTEEVENTS.csv'),
        sep=','
    )
    df_mimic_note.index = df_mimic_note["ROW_ID"].values

    df_mimic_presc = pd.read_csv(
        os.path.join(mimic_path, 'PRESCRIPTIONS.csv'),
        sep=','
    )
    
    df_mimic_patient = pd.read_csv(
        os.path.join(mimic_path, 'PATIENTS.csv'),
        sep=','
    )
